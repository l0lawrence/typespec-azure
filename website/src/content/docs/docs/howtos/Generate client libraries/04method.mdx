...
:::note
`@convenientAPI` and `@protocolAPI` must be applied with a language scope of `"java"` or `"csharp"`. Using them without a scope or with an invalid scope produces a `decorator-requires-scope` warning.
:::
